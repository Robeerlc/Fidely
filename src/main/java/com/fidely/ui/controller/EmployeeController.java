...
import com.fidely.domain.dto.response.EmployeeResponse;
import com.fidely.domain.service.EmployeeService;
...
public class EmployeeController {
    ...
    public ResponseEntity<String> createEmployee(@Valid @RequestBody CreateEmployeeRequest request) {
        ...
        return ResponseEntity.ok("Empleado " + request.name() + " registrado correctamente para tu negocio.");
    }
    ...
    public ResponseEntity<List<EmployeeResponse>> listEmployees() {
        ...
        return ResponseEntity.ok(employeeService.getEmployees(ownerEmail).stream()
                .map(e -> new EmployeeResponse(e.getId(), e.getName(), e.getEmail()))
                .toList());
    }
